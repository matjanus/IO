        ...
        <main>
            ...
            <div class="newExe-forms">
                <form action="/updateExercise" method="POST">
                    ...
                    <input type="text" id="name" name="name" value="<?= htmlspecialchars($exercise->getName()) ?>" required>

                    <label for="instruction">Instrukcja:</label>
                    <textarea id="instruction" name="instruction" rows="8" required><?= htmlspecialchars($exercise->getFields()['instruction']) ?></textarea>

                    <label for="hint">Podpowiedź:</label>
                    <textarea id="hint" name="hint" rows="2"><?= htmlspecialchars($exercise->getFields()['hint']) ?></textarea>

                    <label for="result">Oczekiwany wynik:</label>
                    <textarea id="result" name="result" rows="2"><?= htmlspecialchars($exercise->getFields()['result']) ?></textarea>

                    <label for="solution">Rozwiązanie:</label>
                    <textarea id="solution" name="solution" rows="8"><?= htmlspecialchars($exercise->getFields()['solution']) ?></textarea>

                    <button type="submit" class="set-button">Zapisz zmiany</button>
                </form>
            </div>
        </main>
    </div>
